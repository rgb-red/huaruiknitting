							...
							<form class="layui-form" action="" method="POST">
								...
								<div class="layui-form-item">
									...
									<div class="layui-col-md3 product-img">
										...
										<a href="javascript:;"><img src="<?=$front_url . "/uploads/p_cover/" . $id . ".jpg"?>" width="220px"></a>
										<?php else:?>
...
<script>
	...
	layui.use('layedit', function(){
		...
		layedit.set({uploadImage: {url: '/ajax/news_upload_image'}});
	...
	$("#save-btn,#draft-btn").click(function(){
		...
		if(!title || !product_classify || !en_title){
			layer.msg("*为必填项，请勿留空！")
	...
	function alert1(cover, brief, en_brief, data){
		if(!cover && $(".product-img img").attr("src") == "/images/addimg.jpg"){
			layer.alert("未上传新闻封面，将使用默认图片作为封面！", function(index){
				layer.close(index);
	...
	function alert2(brief, en_brief, data){
		if(brief == "" || en_brief == ""){
			layer.alert("未填写简介，将根据新闻详情生成简介内容！", function(index){
				layer.close(index);
				do_ajax(data);
			});
		}else{
				...
				success: function(data){
					if(data == 0){
						...
						layer.alert(msg2, function(){
							...
							//window.location.href = "/admin/edit_news?id=" + data;
						})
					}
				},
				error: function(){
