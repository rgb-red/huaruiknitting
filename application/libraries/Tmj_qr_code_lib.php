...
class Tmj_qr_code_lib {
	...
	* Return block number 1
	...
	/**
	 * 图片生成方法
	 *    图片像素计算公式：图片宽/高的像素 = （25 + 2 × 边框点数） × 每点像素
	 * 
	 * @param $frame array() 二维码的二维数组
	 * @param $pixelPerPoint int 每点的像素
	 * @param $outerFrame int 边框的点数
	 * 
	 * @return 
	 * 
	 */
	...
	{
		// 二维码的点数： 25×25
		$h = count($frame);//二维码高的点数：25
		$w = count($frame[0]);//二维码宽的点数：25
            
		// 图片的点数：二维码的点数 + 边框的点数
		$imgW = $w + 2*$outerFrame;//图片宽的点数 = 二维码宽的点数 + 2个边框的点数 
		$imgH = $h + 2*$outerFrame;//图片高的点数 = 二维码高的点数 + 2个边框的点数
	 ...
	 * jpg二维码
	 *    图片像素计算公式：图片宽/高的像素 = （25 + 2 × 边框点数） × 每点像素
	 *    示例：$succ = $this->sk_qr_code_lib->jpg ( $url, $tmp_path, 'L', 4.138, 4.138 ); 则生成的图片宽/高像素 = （25 + 2 × 4.138） × 4.138 = 138.40305
	 * 
	 * @param $str string 字符串 要生成二维码的字符串
	 * @param $filename string 文件名
	 * @param $eclevel string <ul><li>L : About 7% or less errors can be corrected.</li><li>M : About 15% or less errors can be corrected.</li><li>Q : About 25% or less errors can be corrected.</li><li>H : About 30% or less errors can be corrected.</li></ul>
